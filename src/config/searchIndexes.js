...
async function createSearchIndexes() {
  try {
    ...
    async function safeCreateIndex(collection, indexSpec, options = {}) {
      try {
        ...
        if (!indexExists) {
          ...
          console.log(
            `✅ Created index: ${options.name || JSON.stringify(indexSpec)}`
          );
        } else {
          console.log(
            `⏭️ Index already exists: ${options.name || JSON.stringify(indexSpec)}`
          );
        }
      } catch (error) {
        if (error.code === 85) {
          ...
          console.log(
            `⚠️ Index conflict (ignored): ${options.name || JSON.stringify(indexSpec)}`
          );
        } else {
          console.log(
    ...
    await safeCreateIndex(
      userCollection,
      { isVerified: 1, role: 1, createdAt: -1 },
      { name: 'verified_role_created_index' }
    );
    ...
    await safeCreateIndex(
      profileCollection,
      { primarySport: 1 },
      { name: 'primary_sport_index' }
    );
    await safeCreateIndex(
      profileCollection,
      { experienceLevel: 1 },
      { name: 'experience_level_index' }
    );
    await safeCreateIndex(
      profileCollection,
      { verified: 1 },
      { name: 'profile_verified_index' }
    );
    await safeCreateIndex(
      profileCollection,
      { rating: -1 },
      { name: 'profile_rating_index' }
    );
    ...
    await safeCreateIndex(
      clubProfileCollection,
      { verified: 1 },
      { name: 'club_verified_index' }
    );
    await safeCreateIndex(
      clubProfileCollection,
      { rating: -1 },
      { name: 'club_rating_index' }
    );
