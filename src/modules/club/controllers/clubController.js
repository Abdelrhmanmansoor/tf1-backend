...
exports.verifyNationalAddress = async (req, res) => {
  const { buildingNumber, additionalNumber, zipCode } = req.body;
  const requestId = Date.now().toString(36) + Math.random().toString(36).substr(2);

  // Log incoming request
  console.log(`[NationalAddressVerification] Request ${requestId} started`, {
    timestamp: new Date().toISOString(),
    params: { buildingNumber, additionalNumber, zipCode }
  });

  try {
    if (!buildingNumber || !additionalNumber || !zipCode) {
      console.warn(`[NationalAddressVerification] Request ${requestId} failed: Missing fields`);
      return res.status(400).json({
        success: false,
    ...
    const apiUrl = 'https://apina.address.gov.sa/NationalAddress/v3.1/address/address-verify';

    // Security check: Warn if using mock key in production
    if (process.env.NODE_ENV === 'production' && apiKey === 'MOCK_KEY') {
      console.warn(`[NationalAddressVerification] SECURITY WARNING: Running in PRODUCTION mode but NATIONAL_ADDRESS_API_KEY is missing. Using MOCK logic.`);
    }
    ...
    if (apiKey !== 'MOCK_KEY') {
        try {
            console.log(`[NationalAddressVerification] Request ${requestId}: Calling External API`);
            ...
            if (!response.ok) {
                console.error(`[NationalAddressVerification] Request ${requestId}: External API Error ${response.status}`);
                // Handle 401, 500 etc.
            ...
            console.log(`[NationalAddressVerification] Request ${requestId}: External API Success`, apiResponse);
        } catch (apiErr) {
            console.error(`[NationalAddressVerification] Request ${requestId}: External API Exception`, apiErr);
    ...
    const message = isVerified
      ? 'تم التحقق من العنوان الوطني بنجاح'
      : 'تعذر التحقق من العنوان الوطني — سيتم إنشاء الحساب بدون توثيق العنوان.';

    console.log(`[NationalAddressVerification] Request ${requestId} completed. Verified: ${isVerified}`);
    ...
    console.error('Verify national address error:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error during address verification',
      error: error.message
    });
  }
};

// Retry National Address Verification (Updates Profile)
exports.retryVerification = async (req, res) => {
  const requestId = Date.now().toString(36) + Math.random().toString(36).substr(2);
  const { buildingNumber, additionalNumber, zipCode } = req.body;

  // Log incoming request
  console.log(`[NationalAddressVerification:Retry] Request ${requestId} started`, {
    timestamp: new Date().toISOString(),
    userId: req.user._id,
    params: { buildingNumber, additionalNumber, zipCode }
  });
  ...
  try {
    if (!buildingNumber || !additionalNumber || !zipCode) {
      console.warn(`[NationalAddressVerification:Retry] Request ${requestId} failed: Missing fields`);
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: buildingNumber, additionalNumber, zipCode'
      });
    ...
    // Security check: Warn if using mock key in production
    if (process.env.NODE_ENV === 'production' && apiKey === 'MOCK_KEY') {
      console.warn(`[NationalAddressVerification:Retry] SECURITY WARNING: Running in PRODUCTION mode but NATIONAL_ADDRESS_API_KEY is missing. Using MOCK logic.`);
    }
    ...
    if (apiKey !== 'MOCK_KEY') {
        try {
            console.log(`[NationalAddressVerification:Retry] Request ${requestId}: Calling External API`);
            const params = new URLSearchParams({
              Buildingnumber: buildingNumber,
              Additionalnumber: additionalNumber,
              Zipcode: zipCode,
            ...
            if (!response.ok) {
                console.error(`[NationalAddressVerification:Retry] Request ${requestId}: External API Error ${response.status}`);
                 return res.status(response.status).json({
                    success: false,
                    message: 'National Address Service Unavailable',
                    code: 'SERVICE_ERROR'
            ...
            apiResponse = await response.json();
            console.log(`[NationalAddressVerification:Retry] Request ${requestId}: External API Success`, apiResponse);
        } catch (apiErr) {
            console.error(`[NationalAddressVerification:Retry] Request ${requestId}: External API Exception`, apiErr);
             return res.status(503).json({
                success: false,
                message: 'National Address Service Unavailable',
                code: 'SERVICE_UNAVAILABLE'
    ...
    const isVerified = apiResponse.success && (apiResponse.addressfound === true || apiResponse.addressfound === 'true');
    console.log(`[NationalAddressVerification:Retry] Request ${requestId} completed. Verified: ${isVerified}`);
    ...
    profile.location.nationalAddress = {
      buildingNumber,
      additionalNumber,
      zipCode,
      isVerified,
    ...
    const message = isVerified
      ? 'تم التحقق من العنوان الوطني وتحديث الملف بنجاح'
      : 'تعذر التحقق من العنوان الوطني — تم تحديث البيانات ولكن الحالة غير موثقة.';
    ...
    res.status(500).json({
      success: false,
      message: 'Internal server error during verification retry',
      error: error.message
    });
  }
};
