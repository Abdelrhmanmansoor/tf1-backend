...
reviewSchema.index(
  { revieweeId: 1, reviewerId: 1, 'relatedTo.entityId': 1 },
  { unique: true }
);
...
reviewSchema.statics.getReviewsForReviewee = async function (
  revieweeId,
  options = {}
) {
      ...
      ? {
          ...
          detailedAverages: {
            professionalism: stats[0].avgProfessionalism
              ? Math.round(stats[0].avgProfessionalism * 10) / 10
              : null,
            communication: stats[0].avgCommunication
              ? Math.round(stats[0].avgCommunication * 10) / 10
              : null,
            expertise: stats[0].avgExpertise
              ? Math.round(stats[0].avgExpertise * 10) / 10
              : null,
            punctuality: stats[0].avgPunctuality
              ? Math.round(stats[0].avgPunctuality * 10) / 10
              : null,
            value: stats[0].avgValue
              ? Math.round(stats[0].avgValue * 10) / 10
              : null,
          },
        }
      : null,
...
reviewSchema.statics.getReviewsByUser = async function (
  reviewerId,
  options = {}
) {
...
reviewSchema.methods.addHelpfulVote = function (userId, voteType) {
  ...
  this.notHelpfulCount = this.helpfulVotes.filter(
